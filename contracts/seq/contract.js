...
var removeExpired = (queue, height) => queue.filter((request) => request?.expires > height);
...
// src/write/mint.js
var { of: of3, fromPromise: fromPromise2 } = src_default;
function mint({ contracts, block }) {
  return (state) => {
    state.pile = Object.fromEntries(
      Object.entries(state.pile).filter((e) => e[1] >= block.height)
    );
    return of3(state.mint_contract).chain(
      (id) => fromPromise2(contracts.viewContractState)(id, { function: "get-queue" })
    ).map(({ result }) => result).map((queue) => removeExpired(queue, block.height)).map((queue) => notInPile(state, queue)).map(combineQuantitiesByTarget).map(
      ({ requests, combinedTargets }) => updateBalances2(combinedTargets, requests, state)
    ).map((queue) => {
...
var updateBalances2 = (combinedTargets, requests, state) => {
  for (let i = 0; i < combinedTargets.length; i++) {
    const req = combinedTargets[i];
    const target = req.target;
    const qty = req.qty;
    const balance2 = state.balances[target] || 0;
    state.balances[target] = balance2 + qty;
  }
  return requests;
};
