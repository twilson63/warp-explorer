...
// src/write/create-mint.js
function createMint({ block, transaction }) {
  return (state, action) => {
    return of({ state, action, block, transaction }).chain(
      ce(
        roundDown(transaction.reward / state.divisibility) < 1,
        "You must mint at least 1 feron."
      )
    ).map(({ state: state2, action: action2, block: block2, transaction: transaction2 }) => {
      state2.requests.push({
        tx: transaction2.id,
        target: action2.caller,
        qty: roundDown(transaction2.reward / state2.divisibility),
        expires: block2.height + 720
      });
      return state2;
    }).fold(
      (msg) => {
...
// src/read/get-queue.js
function getQueue(state, action) {
